...
| `penetrated` | 穿透   | （仅对于变量或临时变量）绕过对象索引机制，将变量或临时变量直接存储到内存中                         |
...
> [!NOTE]
> 异步与等待章节的内容为**编译时可选项**，允许在编译时剥离对异步与等待的支持。

异步是指程序在执行某些操作时，不会阻塞主线程，而是将这些操作放到后台执行，待操作完成后再通知主线程继续处理。NovaSharp 通过 `async` 修饰符和 `await` 关键字支持异步编程。

#### 异步函数

异步函数是一种特殊类型的函数，用于支持异步操作。异步函数在执行时不会阻塞主线程，而是可以在等待耗时操作（如网络请求、文件读写等）时让出控制权，待操作完成后再继续执行后续代码。

要定义异步函数，需要在函数声明前加上 `async` 修饰符。异步函数可以返回 `void`、`Task` 或 `Task<T>` 类型。例如：

```novasharp
public async Task<int> DownloadDataAsync(string url)
{
    // 异步操作
    int result = await Network.DownloadAsync(url); // 此处仅为示例，实际不存在Network.DownloadAsync()，若需了解相关内容请查阅标准库。
...
在异步函数内部，可以使用 `await` 关键字等待一个异步操作完成。`await` 后面跟随一个可等待的对象（如 `Task` 或 `Task<T>`）。当遇到 `await` 时，函数会挂起执行，直到异步操作完成后再继续。

```novasharp
public async Task ProcessAsync()
{
    await DownloadDataAsync("https://example.com");
...
异步函数中的异常可以通过常规的 `try-catch` 语句进行捕获：
...
>
> - 只能在异步函数中使用 `await`。
> - 异步函数的返回类型必须为 `void`、`Task` 或 `Task<T>`。
...
`penetrated` 修饰符用于指示变量或临时变量绕过对象索引机制，直接存储到内存中。也算是 NovaSharp 的一大特色。

有关对象索引机制的内容，详见[对象索引与索引单元](#对象索引与索引单元)。

当一个变量被声明 `penetrated` 时，它将绕过对象索引机制，直接存储到内存中。这意味着对该变量的访问将不再通过索引单元，而是直接访问其在内存中的位置。
