...
当继承一个类时，子类将自动获得父类的所有公共和受保护成员。子类可以通过 `base` 关键字访问父类的成员。
...
注意，这不会引入子命名空间下的子命名空间。在上述示例中，如果 `Standard.IO` 下还有其他子命名空间，它们不会被自动引入。

NovaSharp 允许**在所有命名空间之外**定义函数、变量或对象，这些成员将**不属于任何命名空间**，并**可以在全局范围内被访问**。此时，它们的**可访问性修饰符将被忽略**，因为可访问性修饰符**不再适用**于它们。
...
NovaSharp 将多份代码整合为一个项目，由项目配置文件管理它们。
